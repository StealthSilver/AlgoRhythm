...
export function HeroGridAnimations() {
  return (
    <div className="hero-grid-bg hero-grid-mobile relative grid w-[min(100vw,1280px)] max-sm:ml-auto max-sm:w-[168vw] max-sm:translate-x-[12vw] max-sm:translate-y-[4.5rem] max-sm:gap-0 shrink-0 grid-cols-4 grid-rows-3 opacity-[0.55]">
      {HERO_ANIMATIONS.map(({ id, Component }) => (
        <div
          key={id}
          className="relative flex min-h-[210px] max-sm:min-h-[284px] max-sm:p-5 items-center justify-center overflow-hidden sm:min-h-[230px] lg:min-h-[260px]"
        >
